...
class IngestionTaskManager:
    ...
    def __init__(self):
        ...
        self.task_retention_minutes = 5  # Keep completed tasks for 5 minutes
    ...
    def get_task(self, task_id: str) -> Optional[IngestionTask]:
        """Get task by ID and cleanup old tasks."""
        # Cleanup old completed tasks (older than retention period)
    ...
    def _cleanup_old_tasks(self):
        ...
        now = datetime.now()
        cutoff_time = now - timedelta(minutes=self.task_retention_minutes)
        
        tasks_to_remove = []
        for task_id, task in self._tasks.items():
            # Only cleanup completed tasks
